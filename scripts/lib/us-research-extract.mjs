 ...
 *
 * 실측(2026-09)으로 확인한 한국 증권사 미국 리포트의 실태 — 자체 목표가를
 * 아예 안 내고 컨센서스만 적는 경우가 대부분이며 표기 형태도 브로커마다
 * 다르다(AAPL 리포트로 직접 확인):
 *  - 키움: "목표주가 컨센서스: $243.41"
 *  - 신한: "목표주가 (LSEG, 컨센서스) 248.8 달러" — 출처(LSEG) 명시.
 *  - 하나: "TP(컨센서스) 308.9 USD"
 *  - 현대차: "컨센서스 TP  USD 323.27" — 숫자가 USD 뒤에 옴.
 *  - 한화: "컨센서스 목표주가/투자의견 323.27" — 목표가·의견이 "/"로 결합.
 *  - KB: "Consensus target price (USD)\n325.4" — 라벨과 숫자가 줄바꿈으로 분리.
 *  - 미래에셋: "Refinitiv\n평균목표주가 USD 70.5" — 출처(Refinitiv) 다음 줄.
 *  - 유진: "투자의견 NR" — 명시적 무등급.
 *  - 한화(면책): 목표주가·투자의견이 면책 문구에도 등장("…의견을 제시합니다").
 *
 ...
 * 라는 단어가 패턴 안에 반드시 들어있게 설계). \s 는 개행도 매칭하므로
 * "라벨\n숫자"처럼 줄바꿈으로 떨어져 있는 PDF 레이아웃(KB 등)도 잡힌다.
 */
const CONSENSUS_TARGET_PATTERNS = [
  ...
  // "컨센서스 목표주가: $243.41" / "컨센서스 목표주가/투자의견 323.27" (한화)
  /컨센서스\s*목표\s*주가(?:\s*\/\s*투자의견)?\s*[:：]?\s*\$?\s*([\d,]+(?:\.\d+)?)\s*(?:달러|USD)?/i,
  // "TP(컨센서스) 308.9 USD" (하나)
  /TP\s*\(\s*컨센서스\s*\)\s*([\d,]+(?:\.\d+)?)\s*(?:달러|USD)/i,
  // "컨센서스 TP  USD 323.27" (현대차) — 숫자가 USD 뒤에 오는 순서도 허용
  /컨센서스\s*TP\s*(?:USD|달러)?\s*([\d,]+(?:\.\d+)?)/i,
  // "Consensus target price (USD)\n325.4" (KB)
  /consensus\s*target\s*price\s*\(?\s*(?:USD|달러)?\s*\)?\s*([\d,]+(?:\.\d+)?)/i,
  // "Refinitiv\n평균목표주가 USD 70.5" (미래에셋)
  /Refinitiv\s*평균\s*목표\s*주가\s*(?:USD|달러)?\s*\$?\s*([\d,]+(?:\.\d+)?)/i,
];
