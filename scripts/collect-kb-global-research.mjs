 ...
 *  - foldertemplate 로 시장이 구분된다 - "미국" 이 들어간 것만 쓴다
...
console.log(`▶ KB증권 해외주식 리포트 수집: 최근 ${DAYS}일`);
...
const up = await fetch(IMPORT_URL, {
  method: "POST",
  headers,
  body: JSON.stringify({ items: collected, source: "KB증권", market: "us" }),
});
