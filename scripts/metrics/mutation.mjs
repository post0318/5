...
export const MUTATIONS = [
  ...
  // 판본 판정 decimals 교차 확인(2026-09-26, 오너 승인 2단계) — 검증기 기준값이 decimals 독립 판정과 옛 규칙이 같은 행에서만 독립 통과
  { id: "M02c", cat: 2, sym: "MCD", col: "2023Y", what: "재게시를 재작성으로 — 영업이익 11,646,700,000 → 11,647,000,000(2025·2026 10-K 가 백만 단위 d-6 으로 다시 실은 값)", origin: "MCD 2023 영업이익 — 2024 10-K d-5 정밀값을 나중 10-K 가 d-6 으로 재게시(decimals 판정 = 재게시). 이 행은 종전 공통모드(판본 선택 = 앱 규칙)였고 decimals 독립 확인으로 독립 통과가 된 행",
    expect: "A", plants: [{ op: "replace", from: 11646700000, to: 11647000000 }] },
  { id: "M03c", cat: 3, sym: "DELL", col: "2024Y", what: "재작성을 재게시로 — 세전이익 4,087,000,000 → 3,887,000,000(재작성 전 2024-03-25 10-K 값 유지)", origin: "DELL FY2024 법인세 재작성(10-K 2025-03-25, 두 판본 모두 d-6 → decimals 판정 = 재작성). 지시의 AXP FY2017 충당금(2,759 → 2,760, d-6 동일)은 검증기 검사 범위 밖(충당금 검사 없음·AXP 열 2021Y~)이라 같은 유형의 DELL 검사 대상 줄로",
    expect: "A", plants: [{ op: "replace", from: 4087000000, to: 3887000000 }] },
  ...
  { id: "M12b", cat: 12, knownGap: "현재 주식수 대조 검사 없음 — 주식수·시가총액 지표를 닫을 때 SEC 최신 표지(dei) 기준일 대조로 막는다(docs/metrics/mutation.md 새 구멍 처리)", sym: "KO", col: "현재", what: "현재 주식수 +1,000주 4,302,549,243 → 4,302,550,243(TTM 스냅샷)", origin: "현재 주식수 = EDGAR → 인포맥스 보정 경로",
