...
// 있었다.
//  (1) 「6. 금리정책」 섹션이 이미 연준·한국은행·일본은행을 전담하는데
 ...
 * "경기"도 추가(오너 지시 2026-09-21, "5. 경제" 섹션 신설). 실측: 「경기」
 * 계열은 병합이 안 되다 보니(관세·중국·금·구리·BDI 가 서로 포함 관계가
 * 없음) 최고점이 0.25 안팎에 머물러 다른 계열(0.5~0.7 대)에 항상 밀렸다
 * ("경기는 안나오네" — 경쟁시켜 봤자 매번 진다). 전용 섹션으로 옮겨
 * 경쟁 자체를 없앤다.
 */
const EXCLUDED_FAMILIES = new Set(["통화정책", "경기"]);
