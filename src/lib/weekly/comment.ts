...
5. **snapshot 의 pct/diffBp 를 말로 그대로 옮기기만 하는 코멘트는 절대
   ...
   - 좋은 예: 그 자산이 issues 중 하나와 실제로 연결되면 그 이슈의 원인을
     짧게 인용한다(예: "국제유가 급등 여파로 에너지 비용 부담 반영").
   - **그 자산이 어느 issues 항목과도 실질적으로 안 엮이면 빈 문자열("")
     로 남긴다.** 엮을 근거가 없는데 등락률만 재진술하느니 비워두는 게
     낫다 — 19개 중 몇 개만 채워져도 된다, 전부 채우려 하지 마라.
