...
| L2 EOD 시세 | `yahoo-finance2` (주, 2026-09-24 오너 결정 — Stooq 연결 불가), Stooq (마지막 폴백, 먹통이면 30분 차단). 한국은 KRX 우선 | 인앱 |
...
- **크롤링은 어떤 시나리오에서도 금지**: FnGuide(`robots.txt Disallow: /`),
  ...
  - **검증용 조회 예외 (오너 승인 2026-09-23, 2026-09-24 "1회" 제약 삭제)**: 재무 숫자
    ...
    **검증 스크립트 자동 대조 허용(오너 결정 2026-09-24 — "검증 스크립트에만 넣기")**: 재무 검증
    스크립트(`scripts/verify-financials.mjs`, 관리자 화면용 GitHub Actions 포함)에 한해 StockAnalysis·
    FnGuide 를 자동 대조 소스로 쓴다. 앱 배포본·수집기·DB 에는 넣지 않는다(결과 판정·차이만 저장).
    실측: StockAnalysis 는 브라우저 UA 로 SvelteKit 데이터 엔드포인트가 열림, FnGuide 는 신버전
    `wcomp.fnguide.com/CompanyInfo/*` 가 서버렌더(옛 `comp.fnguide.com/SVO2` 는 폐지), 인베스팅닷컴·
    MarketScreener 는 봇 차단(403)이라 스크립트 불가 — 불일치 항목만 브라우저로 개별 확인.
  ...
  - **예외 2건 (개인용, 오너 명시 승인, 2026-09)**: 신한투자증권 "기업분석"
    ...
    **로컬 전용 스크립트** (`scripts/collect-shinhan-research.mjs`, 하루 1회
    ...
    - **NH투자증권 추가(오너 확인, 2026-09)**: `www.nhsec.com` 은 레거시
      ...
      - **카테고리 정리 — ESG 제외·비상장 인사이트 분리(오너 지시,
        2026-09-24 — "nh랑 삼성증권도 카테고리별 분류를 부탁한다")**: 국내
        게시판 "산업" 라벨 65개를 전수 조사한 결과 대부분 정상 업종명이고,
        "[NH ESG Research]"(공용 필터 `isEsgContent`로 제외 — "esg는
        공통으로 제외처리")와 "[NH 비상장]"(source:"NH투자증권 비상장리서치"
        로 분리 — "국내 비상장은 종목분석 인사이트로 해외 비상장은 그대로
        산업분석으로 유지", 이 게시판은 국내라 대상) 딱 두 라벨만 예외였다.
        같은 조사에서 응답 제목에 이스케이프 안 된 raw 탭 문자가 섞여 나와
        `JSON.parse`가 "Bad control character"로 죽는 버그도 발견해
        함께 수정(`collect-nh-research.mjs`·`collect-nh-overseas-research.mjs`
        둘 다 — 문자열 값의 제어문자를 공백으로 치환 후 파싱).
    ...
    - **KB증권 추가(오너 확인, 2026-09)**: `www.kbsec.com` 리서치보고서
      ...
      - **산업분석/투자전략 수집 추가(오너 지시, 2026-09)**: "종목분석 옆에
        ...
        같은 방식으로 순차 확장 예정.
      - **게시판 구조 전면 정리 — `categoryid` 발견 및 확장(오너 지시,
        2026-09-24 — "kb증권도 키움처럼 게시판별로 정리해서 보여줘" →
        "kb는 별도 코드 없나?")**: `tab`(1~6)은 상위 대분류일 뿐이고,
        응답 항목마다 실린 `categoryid`/`pCategoryid` 필드가 키움의
        `rMenuGb`에 해당하는 진짜 게시판 코드임을 확인(`tab=6` 통합 피드로
        전수 조회해 `foldertemplate`와 대조, 54개 leaf 카테고리 확인) —
        상세 매핑 표는 `collect-kb-research.mjs` 헤더 주석 참고. 이 발견을
        계기로 오너가 게시판별로 최종 분류를 확정(2026-09-24, 전부):
        - **KB데일리**(tab=1, categoryid 79, "자산배분/매크로 > KB데일리")
          — "kb데일리는 산업분석>시황에 해당한다": `category:"산업"`으로
          수집하고 stockName을 고정 라벨 "KB데일리"로 둬
          `MARKET_CONDITION_STOCKNAMES`(`shinhan-research.ts`)에 등록,
          키움 "일간증시전망"과 동일 방식으로 시황 확정 분류.
        - **"한국 투자"(tab=3)** — "한국투자에서 kb전략은 투자전략(주식)에
          해당되고 kb bond는 거시경제>이슈분석에 해당된다. 나머지는
          수집에서 제외한다": 이 탭은 시황코멘트·주식전략("KB
          전략"·"이그전")·채권/크레딧("KB Bond"·"KB Fed Watch"·"KB Credit
          Weekly")·종목컨설팅·기타발간이 섞여 있는데, `categoryid`만으론
          정확히 못 가른다(실측 — 같은 categoryid 안에도 "이그전" 같은
          다른 시리즈가 섞여 나옴) — **docTitle 정확히 일치**로만
          "KB 전략"(`collect-kb-research.mjs`, stockName 고정,
          `STRATEGY_STOCKNAMES`에 등록해 투자전략(주식) 확정)과 "KB
          Bond"(`collect-kb-macro-issues.mjs`, `macro_issues` 컬렉션,
          topic:"이슈분석" 고정)만 골라 수집하고, 시황코멘트·KB Quant·
          이그전·KB Credit Weekly·종목컨설팅·기타발간은 전부 수집 제외.
          **"KB Fed Watch"는 이후 포함으로 정정**(오너 지시, 2026-09-24 —
          "KB Fed Watch는 이슈분석에 포함한다", 처음엔 "나머지는 제외"에
          같이 묶여 빠졌었음) — `collect-kb-macro-issues.mjs`의
          `TAB3_ISSUE_TITLES`에 "KB Bond"와 함께 등록, 둘 다 topic:"이슈분석".
        - **"자산배분/매크로"(tab=2)** — "자산배분/매크로는 거시경제>
          이슈분석에 해당한다. 다만, weekly, 주간 등은 수집대상이 아니다.
          구분값이 환율이거나 FX로 되어있다면 환율분석으로 분류한다":
          `collect-kb-macro-issues.mjs`가 tab=2 전체(대체투자/자산배분/
          매크로/자산배분기타)를 훑어 제목에 "Weekly"/위클리/주간이 있으면
          건너뛰고(예: "KB Crypto 트래커 | Weekly", "주간 주식시장 동향 및
          전망"), 나머지는 제목 또는 폴더 경로 말단이 "FX"/"환율"이면(예:
          폴더 "...매크로>환율", 제목 "FX 전략", 실측 9건/3개월)
          topic:"환율분석", 그 외(실측 145건/3개월)는 topic:"이슈분석"으로
          분류해 `macro_issues`에 전송한다.
        - **산업/기업(tab=5) 재정리** — "산업/기업 중 코드나 종목명인
          경우 종목분석에 해당되고 그 외는 산업분석에 해당한다. 다만,
          포트폴리오, 추천종목, ESG등은 대상에서 제외한다. 또한 IPO나
          비상장인 경우는 종목분석>인사이트에 해당한다": 종목코드 매칭
          (`TITLE_RE`)은 기존 그대로 `category:"기업"`(=종목분석)이고,
          그 외 업종·전략 라벨은 `category:"산업"`(=산업분석)이던 기존
          로직에 두 가지 필터를 추가했다 — ① `EXCLUDE_LABEL_RE`(포트폴리오/
          추천종목/ESG 라벨, 예: "KB 리서치 모델 포트폴리오"·"대형주
          추천종목"·"Global ESG Brief")는 아예 수집하지 않는다(기존엔
          `category:"산업"`으로 수집하던 것을 이번에 제외로 전환). ②
    ...
    - **KB증권 해외주식(미국 종목) 추가(오너 확인, 2026-09)**: 같은 내부
      ...
      - **중국·일본 추가(오너 지시, 2026-09-24 — "kb 중국과 일본도 수집기는
        만들어두고")**: 처음엔 "미국" 폴더만 고르고 나머지(중국/일본/인디아)를
        버렸는데, 같은 tab=4 응답에 이미 다 섞여 있던 걸 `categoryid`로
        정확히 갈라 `market:"ch"`/`"jp"`로 추가했다(키움 CH 게시판과 동일
        원칙 — `ResearchMarketId`로 별도 태깅, "us"에 합치지 않음). 158
        ("해외 투자 > 중국 > 산업/기업")은 "SMIC (688981 CH, 00981 HK)"·
        "텐센트 (00700 HK)" 형식(복수 상장 시 콤마로 병기, 첫 코드만 사용)이면
        `category:"기업"`, "KB China Theme | {테마}"·"KB Asia Monitor" 같은
        시리즈 라벨이면 `category:"산업"`. 86("해외 투자 > 중국 > 전략",
        "KB Asia Market Headline")은 거의 매일 올라오는 아시아 시장 헤드라인
        코멘트라 고정 라벨로 두고 `MARKET_CONDITION_STOCKNAMES`에 등록해
        시황으로 확정 분류(KB데일리와 동일 패턴). 160("해외 투자 > 일본 >
        산업/기업")은 실측 6개월간 2건뿐으로 표본이 적고 제목("글로벌기업+"
        등)도 티커 패턴이 일정치 않아 티커 추출 없이 라벨 고정으로만 수집
        (표본이 늘면 재검토). 미국 전용이던 목표주가·투자의견 PDF 보강
        (`enrichUsResearch`, USD 표기 기준)은 `market==="us"`로 범위를
        명시적으로 좁혔다(중국/일본은 대상 아님).
    ...
    - **키움증권 해외(미국) 산업/기업분석 추가(오너 확인, 2026-09-24)**: 오너가
      ...
      **로컬 스크립트** (`scripts/collect-kiwoom-research.mjs`,
      ...
      - **게시판 추가 2차 — 실제 사이트 내비 검증 완료(오너 확인,
        2026-09-24)**: 위 브루트포스로 찾은 게시판들을 오너가 실제 화면과
        하나씩 대조해 확정 — `CR`은 "기업/산업분석 > 기업분석", `CI`는
        "기업/산업분석 > 산업분석", `SN`은 "기업/산업분석 > 스팟노트".
        여기에 이어서 두 게시판을 추가로 붙였다: `EM`("경제/전략 >
        월간증시전망", 고정 라벨 "키움 월간증시전망"으로
        `shinhan-research.ts`의 `STRATEGY_STOCKNAMES`에 등록해 투자전략
        (주식)으로 분류 — 오너 지시 "월간증시전망은 투자전략(주식)"),
        `IM`("경제/전략 > 중장기증시전망", 같은 방식으로 "키움
        중장기증시전망" 고정 라벨 — 오너 지시 "중장기증시전망은
        투자전략(주식)이다"). **EM 안에 "YYYY년 M월 키움 증시 캘린더"라는
        일정표가 섞여 있어 수집 제외**(오너 지시 — "월간증시전망에서 증시
        캘린더는 수집대상에서 제외한다", `collect-kiwoom-research.mjs`의
        `CALENDAR_RE`).
        - **SD(일간증시전망) — 추가했다가 제외로 정정(오너 지시,
          2026-09-24)**: 한때 "경제/전략 > 일간증시전망"을 `SD`로 추가해
          "키움 일간증시전망" 고정 라벨(시황)로 수집했으나, 오너가
          "sd코드는 삭제한다. 수집하지 않겠다"고 결정 — `BOARDS`에서
          제거했다. `shinhan-research.ts`의 `MARKET_CONDITION_STOCKNAMES`에
          남아있는 "키움 일간증시전망" 항목은 이미 수집된 기존 문서의
          분류용으로 남겨둔다(시황 14일 보존기간에 따라 자연 소멸,
          코드 정리 불필요).
        - **CH(해외증시 > 중국/신흥국) 추가 — market은 "ch"로 별도 태깅,
          기업/산업 둘 다 수집(오너 결정, 2026-09-24, 정정)**: 처음엔 이
          앱의 `MarketId`(종목분석 어댑터 기준, kr/us/jp)에 중국이 없어
          개별 종목 리포트를 "us"로 합치거나 아예 건너뛰는 방식을 검토
          했으나, 오너가 명시적으로 정정 — **"ch는 마켓은 ch다 us분류하면
          안된다. 카테고리는 기업/산업이다"**. 종목분석(EDGAR 등 어댑터)이
          중국을 지원하지 않는 것과 리서치 데이터의 market 태깅은 별개로
          다룬다 — `src/lib/db/shinhan-research.ts`에 `ResearchMarketId
          = MarketId | "ch"`를 새로 두고(`isResearchMarketId()`,
          `ShinhanResearchDoc.market`도 이 타입으로 확장) `/api/cron/
          shinhan-research`의 market 검증도 이걸 쓰도록 교체 — 종목분석
          쪽 `MarketId`·어댑터 레지스트리는 전혀 안 건드린다(블라스트
          반경 최소화). "종목명(TICKER.HK)" 형식(예: "텐센트 (0700.HK):
          텐센트의 Muse는 샤오웨이", "CATL(3750.HK): ...", "비야디
          (1211.HK): ...")은 `category:"기업"`(symbol에 "TICKER.HK" 형식
          그대로 저장), "[중국은 지금] ..." 류 대괄호 라벨은
          `category:"산업"`(`parseChBoard`). **미결**: `/ch/research` 같은
          전용 화면·나브탭은 아직 없어(오너가 이전에 밝힌 "국가 추가할
          예정이다" 의도의 다음 단계) 이 태그가 붙은 문서는 당장 화면에
          노출되지 않는다 — 최소한 "us"/"kr"로 잘못 섞이는 것만 막아 둔
          상태.
      - **국내 인사이트 = 비상장(프리IPO) 리서치로 대체(오너 지시,
        2026-09-24 — "국내는 인사이트가 없다. 따라서 미국은 유지하나
        한국은 인사이트를 비상장 리서치로 대체한다. 먼저 키움증권 CI 중
        비상장 종목은 인사이트로 별도 분류한다")**: 해외 IB 인사이트 탭
        (`INSIGHT_SOURCES`)은 전부 market:"us" 소스뿐이라 국내
        (`/kr/insights`)는 항상 비어 있었다. CI(산업분석) 게시판 중
        "회사명(비상장-IPO예정) : 헤드라인" 형식(실측: "멜콘(비상장-
        IPO예정) : 반도체 미세화 트렌드의 수혜를 누리다", "글로벌테크놀로지
        (비상장-IPO예정) : Micro LED의 전면배치에 따른 수혜 기대" — 상장
        종목이 아니라 키움이 다루는 프리IPO 기업 리포트)을 감지해
        (`UNLISTED_RE`) `source:"키움증권 비상장리서치"`로 별도 전송,
        `INSIGHT_SOURCES`에 이 값을 추가해 같은 인프라
        (`getInsightResearch`/`getIndustryResearch`)를 그대로 재사용한다 —
        market이 "kr"이라 미국 인사이트(market:"us")와 자연히 분리되고,
        일반 산업분석(source:"키움증권") 풀에서도 자동 제외된다. 화면
        (`InsightsBoard`, `app-shell.tsx` 서브내비)도 market==="kr"일 때
        제목·탭 라벨을 "인사이트" 대신 "비상장 리서치"로 바꾼다.
    - **거시경제 "이슈분석"/"환율분석" 탭 신설(오너 지시, 2026-09-24)**:
      ...
      - **환율분석**: 게시판 코드 `FE`(rMenuGbNm "일간환율전망", "09/23
        ...
        환율전망이 대상이 된다"). 실제 사이트 내비는 "경제/전략 >
        환율전망"(오너 확인, 2026-09-24) — 화면 탭 이름은 기존대로
        "환율분석"을 유지한다(이 앱의 고정 라벨, 사이트 메뉴명과 다름).
    ...
    - **삼성증권 해외기업/해외산업 추가(오너 확인, 2026-09-24)**: 오너가 제시한
      ...
      market: "us"` 로 재사용.
      - **카테고리 조사 — ESG 필터만 적용, 비상장은 그대로 유지(오너 지시,
        2026-09-24 — "nh랑 삼성증권도 카테고리별 분류를 부탁한다")**: 두
        게시판 각 최근 30건을 조사한 결과 company2에 "오픈AI(OpenAI, 미국
        비상장): ..."·"앤스로픽(Anthropic, 미국 비상장): ..." 같은 해외
        (미국) 비상장 기업 리포트가 섞여 있었으나, "국내 비상장은 종목분석
        인사이트로 해외 비상장은 그대로 산업분석으로 유지"(오너 지시)에
        따라 **이 소스는 손대지 않는다** — market:"us"라 인사이트 분리
        대상이 아니고 `category:"산업"`으로 그대로 수집(라벨이 다소
        지저분해도(`industryLabelAndHeadline()` 콜론 분리 결과) 의도된
        동작). ETF/ESG 공용 필터(`isEtfOrEtpContent`/`isEsgContent`)는
        기존대로 적용. industry2의 "글로벌 포트폴리오 전략 (N월 N주 차)"
        류는 KB의 "포트폴리오" 제외 규칙과 문자열은 겹치지만 실제로는 매주
...
- **ETF/ETP 리포트는 전 수집기 공통 제외 대상(오너 지시 2026-09-24 —
  ...
  - **ESG도 동일하게 공용 필터로 승격(오너 지시, 2026-09-24 — "esg는
    공통으로 제외처리")**: KB 게시판 정리 중 "Global ESG Brief", NH 게시판
    조사 중 "[NH ESG Research]"가 발견되며 ETF와 같은 성격(이 프로젝트
    범위 밖 주제)임이 확인돼 `isEsgContent()`(같은 모듈)로 승격 —
    ETF/ETP 필터와 똑같이 이미 이 모듈을 쓰던 삼성증권·키움증권·
    GlobalMonitor·KB증권·NH투자증권 수집기에 적용 완료. `getIndustryResearch()`
...
- **외화·IFRS 공시 → USD 환산**(`us/edgar-foreign.ts`, 오너 결정 2026-09-24 "전부 USD 로",
  "다른 사이트 확인 후 적용"). 사이트 조사: USD 환산은 인포맥스(연도별 전체)·Finviz(LTM 만)뿐,
  Yahoo·MarketScreener·StockAnalysis 는 원통화 표시. **인포맥스 방식**을 따른다 — 손익·현금흐름·
  주당이익 = 그 기간 평균 환율, 재무상태표 = 기말 환율(Yahoo 일별 환율; 인포맥스 역산 환율과
  0.1~0.3% 일치). companyfacts 로더에서 1회: IFRS 개념 → us-gaap 이름 매핑(TSM·SPOT, IFRS 는 리스를
  차입금에 포함 — 한국 규칙과 같음), 외화 → USD, 20-F USD "편의 환산" 태그는 버림(단일 환율).
  ADR 은 종목 단위(`toAdrBasis`)로 주식수 ÷비율·주당 값 ×비율(ADR 1주 기준 — 인포맥스·Yahoo·Finviz 관행).
  Yahoo **예상치**: 매출은 재무 통화, EPS 는 ADR(비율≠1)이면 이미 ADR 1주당 USD·1:1 상장이면 재무
...
- **분할 계수**(`splitFactorsByYear`) 후보에 25·30·40·50·100 추가(CMG 50:1). 3:2 는 증자와 구분이
  어려워 제외 — 검증 도구가 Yahoo 분할 이력으로 따로 잡는다.
- **매출 = 총매출 `Revenues` 우선**(오너 결정 2026-09-24): 손익계산서 첫 줄. 고객계약 매출(ASC 606)을
  먼저 쓰면 WMT 회원비·BE 전력판매(리스) 매출이 빠져 인포맥스·Yahoo·SEC 총매출보다 1~7% 작았다.
- **EBIT 근사(영업이익 태그 없는 회사)**: 세전이익 + 이자비용 − **지분법 이익**(세전이익 태그가 지분법을
  포함하는 계열일 때만).
- **총수익 안의 비영업 수익 분리**(`us/edgar-revenue-dims.ts`, 영업이익 태그 없는 회사만, 2026-09-24):
  XOM·CVX 는 손익계산서 첫 줄이 "매출 + 지분법 이익 + 기타수익" 합계(`Revenues`)다. ① XOM — 10-K·10-Q
  원본의 `Revenues[ProductOrServiceAxis]` 지분법·기타수익 멤버를 빼고(10-Q 8건 읽음 → 최근 약 13개 분기),
  ② CVX — 회사 고유 지분법·기타수익 태그 합이 "총수익 − 고객계약 매출"과 맞는 해가 2개 이상이면 구조
  확인으로 보고 **모든 기간** 고객계약 매출을 쓴다(부분집합 합을 값으로 쓰면 일부 기간만 채워져 LTM 에서
  총수익과 섞였다 — CVX LTM 하이라이트 2,152.61억 vs 손익계산서 2,106.62억). 차이가 총수익 1% 미만이거나
  연간 고객계약 매출 > 총수익인 해가 있으면 적용 안 함(OXY — `Revenues` 가 "Net sales" 자체, Yahoo 도 총수익).
  영업이익(EBIT 근사)은 이 비영업 수익을 뺀다. XOM 매출이 Yahoo·StockAnalysis·MarketScreener·인포맥스와
  일치, CVX 는 Yahoo 와 일치. 검증 도구도 10-K 원본을 따로 읽어 같은 기준으로 대조한다(`nonopSplitFromInstances`).
- **영업이익 소계가 없는 손익계산서 = 공시 계산 구조**(`us/edgar-is-structure.ts`, 오너 결정 2026-09-24 "DIS·
  FOXA 둘 다 수정"): 10-K·10-Q 의 `_cal.xml` 에서 "세전이익 = Σ 하위 줄" 을 읽어 영업외 항목(이자·지분법·
  영업외손익·투자손익 — 하위 트리 전체, 영업외 합계에서 멈춤)을 빼 영업이익으로 쓴다. 기간마다 하위 줄 합 =
  세전이익(0.5%)인 기간만(값은 최근 제출분, 매출 태그가 바뀐 해는 동의어로 대입) — 안 맞으면 기존 근사.
  **DIS 는 `OperatingIncomeLoss` 가 부문 주석에만 있어**(부문 영업이익 합계 — 본사비·구조조정·손상·인수
  무형상각 제외) 앱이 그걸 회사 영업이익으로 읽었다(2023 128.63억 vs 구조 51.00억) — 손익계산서 계산 구조에
  그 태그가 없으면 옮겨 두고(`OperatingIncomeLossSegmentNoteOnly`) 합성값을 쓴다. 구조조정·손상 줄은 영업
  항목(GAAP) — Yahoo 는 이 줄을 빼서(DIS 2023 89.92억) 손상이 큰 해에 차이 난다(NVDA·WMT 일회성과 같은 원칙).
  **독립 감사 후 보강(2026-09-24)**: 영업외 항목은 **정확한 개념명 목록**(부분일치 정규식이 은행 순수익·순이자
  이익·보험 투자수익을 영업외로 잡아 GS −364억·JPM −1,099억·TRV 절반 영업이익을 냈다), 매출 노드 아래로는
  내려가지 않음, **금융·보험·부동산(SIC 6000~6799) 미적용**. 총수익 분리 회사(XOM)의 분리값이 없는 옛 연도는
  종전 근사. 원본 판독이 한 건이라도 실패하면 분리 전체 미적용(LTM 에서 정의가 섞이지 않게).
- **금융·보험업 영업이익 근사 = 세전이익**(이자 가산 안 함, `financialSector`): 증권사(GS·SCHW — 은행
  레이아웃 아님)에 "세전 + 이자비용"을 적용해 영업이익이 세전의 4배(GS 2025 886.66억 vs 218.52억)였다.
  검증 도구의 새 괴리 검사(영업이익↔세전이익 차이 ≤ 매출 25%)로 발견.
- **손익계산서 주석 "일회성비용(구조조정·손상차손·위약금·합의금 등)"**(`us/edgar-oneoff.ts`, 오너 결정
  2026-09-24 — 항목별로 나누지 않고 한 줄, "손익계산서 별도 줄만"): 10-K·10-Q 계산 구조의 손익계산서 줄 중
  개념명이나 **회사 라벨(모든 역할)**이 일회성 항목인 줄의 합. 표준 태그 합산은 포함관계로 이중 합산되고(DIS
  3,836·3,892·3,128) 회사 고유 태그(NVDA Arm 위약금 13.53억)를 놓쳐서 쓰지 않는다. 금액은 us-gaap 은
  companyfacts, 회사 고유 태그만 인스턴스. 기간마다 그 기간을 담은 가장 최근 10-K(손익계산서 3개 연도 =
  2.2년 안). 다른 비용 줄에 섞인 금액(WMT 소송 합의금 — 판관비 안)은 못 잡아 빈칸. 영업이익에 이미 반영된
  금액이라 합계에 영향 없음. 실측: DIS 2023 3,892·FOXA 2023 1,182·NVDA FY2023 1,353.
- **NFLX 콘텐츠 상각은 감가상각비에 포함**(`us/edgar-content.ts`, 오너 결정 2026-09-24): 표준 태그가 아니라
  10-K·10-Q 원본의 회사 확장 태그(현금흐름표 본표 줄)에서 읽는다. **NFLX 만** — WBD·ROKU 에 같은 규칙을
  적용하면 인포맥스와 더 벌어졌고(FactSet 이 회사마다 조정 기준이 다름), DIS 는 주석에만 있음. DIS·WBD·
  ROKU 의 인포맥스 EBITDA 차이는 미수정으로 분류(docs/verification-status.md).
- **매출 태그 목록 끝에 `RevenuesNetOfInterestExpense`**(증권사·투자은행 GS·MS 순수익 — 없으면 GS
  연도 열 0개, MS 는 2010~2014 에 멈췄다). 은행 레이아웃 TTM 매출도 순수익(JPM 개요 PSR 9% 차이 해소).
- **금융 자회사 보유사(GM·F·CAT·DE)**: EV 만 비우고 차입금·현금은 연결 기준으로 표시(대차대조표
  주석과 같은 값 — 예전엔 하이라이트만 빈칸).
- **손익계산서 희석 EPS = `fyEps`·`ltmEps`(하이라이트와 같은 함수)** — 따로 계산하던 경로가 LTM 을
  Yahoo 주식수로 나눠 0.2~5% 갈렸고, MCD 는 단위 오류 주식수로 1,003만 달러를 냈다.
...
- **본표 기준 원칙(2026-09-24, 3개 외부 소스 교차 대조로 확인)** — 태그 우선순위로 고르면 본표 줄과 주석 태그가
  섞여 이중 합산·누락이 난다. 10-K·10-Q 계산 구조(`_cal.xml`, 없으면 `.xsd` 내장 — MSFT·ORCL·IBM 2026~)의
  **본표 줄**을 기준으로 한다. 회사 고유 태그 줄은 공시 원본(인스턴스)에서 값을 읽고, 원본은 필요할 때만 받는다
  (2MB 넘어 Next 캐시에 안 들어감).
  - **총차입금** = 대차대조표 부채 트리의 차입금 줄 합(`edgar-bs-structure.ts`) + 본표에 유동 차입금 줄이 없으면
    주석 유동분(GOOG·AMZN) + 본표에 리스 줄이 없으면 주석 금융리스. 운용·금융리스 합산 줄(VRT)은 뺀다. 부문
    차원으로만 공시된 날짜(CAT)는 기존 태그 규칙. AMAT·IBM(유동성장기부채 이중)·TSLA(유동분 누락) 수정.
  - **감가상각비** = 현금흐름표 영업활동 조정 항목의 감가상각·상각 줄 합(`edgar-cf-structure.ts`, `pickDa` 가 우선).
    표준 개념은 **개념명으로만** 판정(AMD 라벨 파일 오류), 고유 개념은 라벨. 제외: 사채할인·발행비, 주식보상,
    **운용리스 사용권자산 상각**(임차료 성격 — Yahoo·인포맥스는 포함, PEP 차이 원인), 콘텐츠(NFLX 는 edgar-content),
    계약획득원가·고객 인센티브(V 157억)·재고 공정가치 조정·연금 상각. 주 라벨이 "Depreciation…"/"Amortization of
    intangible…" 로 시작하면 제외어가 섞여도 포함(AMZN·ISRG).
  - **소계 없는 손익계산서**(`edgar-is-structure.ts`): 회사 고유 줄도 원본에서 읽고, 고유 태그 중 **말단 줄의 라벨이
    정확히 "기타수익·비용"** 형태일 때만 영업외(IBM `ibm_OtherExpenseAndIncome` — 연금 비영업 비용). 금융사 부문
    영업이익 태그 떼기는 손익·포괄손익 **결합 보고서**도 손익계산서로 인정(MET).
  - **결산일 주식수**(`edgar-shares.ts` `atFiscalYearEnd`): 결산일(±7일) 본표 값을 순서대로 — 유통주식수 →
    자본변동표 `SharesOutstanding` → 자본변동표 보통주 차원(10-K 원본, `edgar-equity-shares.ts` — WMT·BE·META)
    → 발행 − 자기주식(KO·MCD·DAL·GLW·MDLZ) → 발행주식수. 같은 해 가중평균과 1.2배 안(또는 정수 분할배수)인 첫 값.
    예전 표지(제출일 기준)·가중평균 근사는 폴백. 가중평균만 분할 소급된 해(WMT FY2022·23)는 연말 주식수 × 분할배수.
    결산일마다 **그 해의 10-K** 값(공시 당시 기준) — 제출 목록 `recent`(1,000건)에 없는 옛 10-K 는
    `edgar-annual-filings.ts` 가 과거 목록 파일에서 찾는다(Form 4 가 많은 WMT·META).
  - **영업이익 합성 구성요소**(이자·지분법·영업외)도 세전이익과 같은 **최신 제출분**(재작성본)으로 짝짓는다(MET 2021).
...
- **EV 미표시**: 금융업(예수부채·보험계약부채 계정), 금융 자회사 연결(현대차 — 미국과
  같이 최종 방식 보류).
- **LTM 열 재무상태표 = 최신 분기·반기 보고서**(오너 결정 2026-09-24, 미국 MRQ 와 같은 원칙):
  현금·차입금·비지배지분·지배주주 자본(→ EV·순차입금·PBR)은 손익 TTM 의 마지막 분기말
  ("FY2025 + 2026 반기 − 2025 반기" → 2026 Q2) 값. `krLtmBalance()`(dart-ev.ts)가 같은 판정
  규칙을 분기 facts 에 적용해 `getTtm` 스냅샷(`evBridge`·`equity`·`label`)에 싣고, 하이라이트·
  재무분석·개요·유니버스 LTM 열이 모두 그 값을 쓴다. 분기 BS 가 없거나 계정이 비면 연말값으로
  폴백하되 라벨에 적는다("FY2025 (2026 Q2 재무상태표 없음 — 연말값)"). 연도 열은 그대로.
...
- **적자여도 EPS 는 음수로 표시, PER 만 부호 규칙으로 비운다**(오너 지시 2026-09-24 — "적자여도 eps 는
  나오는 것 아닌가, 빈칸이면 다른 것도 빈칸 — 1원칙 일관성"). TTM·폴백 EPS 에서 0 이하를 버리던 곳을
  모두 없앴고, 재무분석 LTM 흐름 지표는 연간값으로 대체하지 않는다(하이라이트와 같이 빈칸). PER(TTM)
  부호 규칙은 전 시장 공통(`multiples.ts`), 적자라서 비운 PER 을 Yahoo PER 로 채우지 않는다.
- 유니버스 PBR 분모 = 지배주주 자본(예전 자본총계 — LG에너지솔루션 31% 차이). 손익계산서 희석 EPS 는
  미공시 해에 기본 EPS 로 채운다(`krEpsByYear` 와 같은 값 — 희석 증권 없으면 정의상 동일).
- **미결**: 재무분석 LTM 의 자산·부채·자본총계 등 나머지 재무상태표 항목은 아직 연말값 — 부채비율 등
  일부 LTM 비율은 분자(분기말)·분모(연말) 기준일이 섞인다.
...
- **2·3층**: `npm run verify:financials -- --symbols=AAPL,WMT` (또는 `--universe`,
  `--sp500`, 한국은 `--market=kr --symbols=…`). 실행 중인 앱 API 를 그대로 불러 사용자가 보는 숫자를 검사한다.
  **2026-09-24 전면 재구축**(오너 — "검증도구가 믿을 수 없다니"): 독립 감사(Opus)가 틀린 값을
  넣어 재현한 결함 — 양쪽 빈칸을 통과로 침(TSM 연도 열 0개가 통과), 앱 대 앱 비교뿐이라 전 화면
  공통 오류(순이익 ×1.1)를 못 잡음, 결산일 연도로 맞춰 52/53주 결산 누락(WEN)을 놓침, 조회 실패·
  검사 0건이 "실패 0", 기준값이 없으면 정의가 다른 값으로 **조용히 대체**(AVGO 우선주배당 포함
  순이익 → "공시 자체 불일치" 오판) — 를 모두 고쳤고, 같은 감사자가 재감사해 원래 시나리오 전부
  잡힘을 확인. 원칙:
  1. 통과는 양쪽 값이 있고 같을 때만. 양쪽 빈칸은 검증불가, **있어야 할 값이 빈칸이면 실패**
     (기대치는 앱이 아니라 SEC 원자료로 정한다 — EBITDA·PBR·PSR·EV·연도 열).
  2. 기준값은 대체하지 않는다. 정의상 같은 값일 때만 쓰고 결과에 사유를 남긴다.
  3. 조회 실패·검사 누락·건너뜀·분할 이력 실패는 전부 기록하고 종료코드 1.
  4. 앱 계산을 재현하지 않는다 — 개요·유니버스는 `/api/cron/verify-row`(실제 계산 함수 결과,
     CRON_SECRET/APP_PASSWORD)로 받는다.
  층: A=SEC 원자료 대조(순이익·매출·EPS(분할계수는 Yahoo 분할 이력으로 독립 산출)·LTM 순이익
  (SEC 분기 공시로 TTM 직접 계산)), B=결산기(SEC 최근 5개 10-K 결산일 ↔ 앱 연도 열), C=화면 간,
  D=항등식·부호 규칙·기대치, E=공시 내부 정합성(공시 EPS × 가중평균 ≈ 보통주 귀속 순이익 —
  A층이 앱=공시를 확인한 경우만 "회사 공시 문제"로 검토 목록), F=외부 대조(검토 목록, 기준일이
  같을 때만 차이 계산 — Yahoo EBITDA·차입금, 인포맥스 주식수). 한국은 원자료(DART) 직접 대조가
  아직 없다(화면 간·항등식·기대치·LTM 기준일만). 결과는 `reports/verify/verify-{market}-{KST}.json`.
  **검증 도구 수정 후에는 독립 재감사(틀린 값 주입 재현)를 받는다 — 도구 작성자가 스스로 승인하지 않는다.**
- 검증용 외부 대조 소스: 인포맥스 종목분석(FactSet — 매출·영업이익·EBITDA·감가상각비·순이익·자산총계·EPS·결산일
  시가총액·분기 합 LTM; "ev희석화수량"은 현금을 빼지 않은 값으로 보여 대조 제외), Yahoo, StockAnalysis(검증 스크립트
  전용), FnGuide(국내, 연결 예정). MarketScreener·인베스팅닷컴은 브라우저 개별 확인만.
- **정확 일치 원칙(오너 지시 2026-09-24)**: "허용치를 좁히는 것보다 완벽하게 일치해야 한다. 틀리면 그 차이를 찾아서
  마무리" + "단순히 한곳만 일치한다고 통과하면 안 된다". A층(SEC)은 허용치 없이 정확 비교(순이익·매출·영업이익·
  세전이익·자산총계), companyfacts 미반영 최신 분기는 검증기가 원본에서 따로 읽어 LTM 도 대조. F층은 지표×기간마다
  3개 소스를 나란히 두고, 차이는 **숫자로 성립할 때만** "원인 확인"(외부 영업이익 − 앱 = 앱 일회성비용 / Yahoo 원통화
  비율 = 환율 / 앱 = SEC(A층 일치) / EBITDA = 영업이익 차 + 감가상각비 차 / 감가상각비 차 = SEC 운용리스 사용권자산
  상각 / 다른 소스가 앱과 정확 일치), 0.05% 이내는 "추정"으로만 표시. 합성 영업이익(SEC 태그 없음)은 미결로 남긴다.
