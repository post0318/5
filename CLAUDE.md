...
- **인플루언서 텔레그램 채널 수집 트리거(2026-09)**: 수집 자체는
  ...
  **트리거만 외부 크론이 `workflow_dispatch` 를 호출**하는 방식이다. GitHub
  `schedule` 이 이 저장소에서 실행률 21%(20시간 기대 29회 중 6회, 최대 공백
  4시간 36분, 실측)에 그쳐 화면이 몇 시간씩 멈춰 보였고, 분 값을 정각에서
  비껴도 개선되지 않았다(지연이 아니라 예약 자체가 누락). 이 채널만 분 단위
  신선도가 의미 있어 예외로 두고, 하루 1회인 리서치 수집기들은 그대로
  `schedule` 을 쓴다. 설정 절차는 `DEPLOY.md` §6 참고. 회당 채널당 50건을
