...
- `i`(fin_sym, 선택): 조립 항등식 불성립 열 `[열키, 매출 경로 불성립[], 매출 외 줄 불성립[]]` — 매출 경로면 그 열 `m.rev` 는 null
  (`revenue.md` §6.1). `ck`(선택): 배치가 "새 정기공시 없음"을 마지막으로 확인한 시각(§5.1). 엔진판 3(2026-09-25)부터.
...
- 트리거: GitHub Actions `.github/workflows/fin-build.yml`(하루 1회 06:10 KST + 수동) → 배포 라우트
  ...
  **저장본 없음 → 엔진판(`ENGINE_VERSION`) 다름 → 저장 후 새 정기공시(제출 목록의 최신 10-K/10-Q accn ≠ `la`)** 인 종목만,
  ...
  종목부터 본다. 워크플로는 응답 `pending` 이 0 이 될 때까지 반복 호출. 로컬 수동: `scripts/fin/build.ts`(지정 종목 강제 재적재).
...
- 조립 항등식 판정 불완전(값 없는 자식 줄)은 매출을 비우지 않는다(`revenue.md` §6.1) — 공시가 줄을 싣지 않은 것과 0 을
  구분할 수 없어서다. 다음 지표를 세울 때 그 줄의 값이 필요한 열이면 판독 원천을 늘려 닫는다.
